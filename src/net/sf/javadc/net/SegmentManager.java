/*
 * Copyright (C) 2001 Ryan Sweny, cabinboy@0wned.org
 *
...
/**
 * <code>DownloadSegmentManager</code> represents a helper class of a
 * <code>DownloadRequest</code>, which allows a file to be downloaded in
 * segmented fashion.
 * 
 ...
 * @author Ryan Sweny
 */
public class SegmentManager implements ISegmentManager {

    private final static Category logger = Category
            .getInstance(SegmentManager.class);
     ...
     */
    public SegmentManager(ISettings _settings) {
     ...
     */
    public DownloadRequest getNextSegment(DownloadRequest dr) {
        ...
        if (hash != null) {
            ...
            if (!dr.isSegment()) {
                ...
                DownloadSegmentList segments = (DownloadSegmentList) hash2segments
                        .get(hash);
                ...
                logger
                        .debug("The DownloadRequest was already a segmented download.");
                return null;
            }

        } else {
            logger.debug("The DownloadRequest did not contain any hash "
                    + "information.");
    ...
    /**
     * Create an empty DownloadSegmentList for the given DownloadRequest
     * instance
     * 
     * @param dr
     * @return
     */
    private DownloadSegmentList createSegmentTemplate(DownloadRequest dr) {

        long size = dr.getSearchResult().getFileSize();

        int templates = (int) Math.ceil((double) size
        ...
        if (templates < 2) {
            ...
            DownloadSegmentList list = new DownloadSegmentList(templates,
                    ConnectionSettings.DOWNLOAD_SEGMENT_SIZE, (int) size, dr
     ...
     */
    public void addListener(SegmentManagerListener listener) {
     ...
     */
    public void removeListener(SegmentManagerListener listener) {
...
/*******************************************************************************
 * $Log: SegmentManager.java,v $
 * Revision 1.8  2005/10/02 11:42:27  timowest
 * updated sources and tests
 * Revision 1.7 2005/09/30 15:59:53 timowest
 * updated sources and tests
 * 

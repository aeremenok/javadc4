...
/**
 * <code>HubClientListener</code> represents a <code>ClientListener</code>
 * extension used by <code>Hub</code> instances to notify registered listeners
 * when client specific browse lists have been downloaded.
 * 
 * @author Timo Westk�mper
 */
public class HubClientListener extends ClientListenerBase {

    private final static Category logger = Category
            .getInstance(HubClientListener.class);
     ...
     * </CODE>
     */
    public HubClientListener(Hub hub) {
        this.hub = hub;
     ...
     */
    public final void browseListDownloaded(IClient client, DownloadRequest dr,
            HostInfo host) {
        try {
            if (client == null) {
            ...
            if (success) {
                hub.fireBrowseListDecoded(hub.getUser(dr.getSearchResult()
                        .getNick()));
    ...
    public final void receivedNick(Client client) {
        ...
        if (ui != null) {
            ...
            logger.error("There is no HubUser mapping available for nick "
                    + client.getNick() + " in Hub " + hub);

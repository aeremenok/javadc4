...
/**
 * <code>HubTaskObjectFactory</code> is a simple Jakarta Commons Pool and
 * PicoContainer based Factory method implementation used via the
 * <code>HubTaskFactory</code>.
 * 
 * @author Timo Westk�mper
 */
public class HubTaskObjectFactory extends BaseKeyedPoolableObjectFactory {

    private final static Category logger = Category
            .getInstance(HubTaskObjectFactory.class);
    ...
    /**
     * Create a HubTaskObjectFactory instance which creates HubTask instances
     * via the given PicoContainer tasksContainer
     * 
     * @param tasksContainer
     *            PicoContainer used to create preconfigured instances of hub
     *            tasks
     */
    public HubTaskObjectFactory(MutablePicoContainer tasksContainer) {
    ...
    public Object makeObject(Object arg0) throws Exception {
        ...
        try {
            Class aclass = Class.forName(ConstantSettings.HUBTASKFACTORY_PREFIX
                    + taskName + ConstantSettings.HUBTASKFACTORY_POSTFIX);

            // retrieve component adapter related to the specific component type
            ComponentAdapter adapter = (ComponentAdapter) componentAdapters
                    .get(aclass);

 ...
 */
public class SStartDownloadTask extends BaseClientTask {

    private static final Category logger = Category
            .getInstance(SStartDownloadTask.class);
     ...
     */
    public SStartDownloadTask(ISettings _settings) {
    ...
    protected final void runTaskTemplate() {
        ...
        if (downloadRequest == null) {
            ...
            if (downloadRequest.isSegment()) {
                ...
                long segmentsize = downloadRequest.getSegment().y
                        - downloadRequest.getSegment().x;

                if (file.length() == segmentsize) {
                    clientConnection
                            .setState(ConnectionState.DOWNLOAD_FINISHED);
                    return;
                ...
                if (file.length() == downloadRequest.getSearchResult()
                        .getFileSize()) {
                    clientConnection
                            .setState(ConnectionState.DOWNLOAD_FINISHED);
                    return;
            ...
            // request the file.
            sendCommand("$Get", downloadRequest.getSearchResult().getFilename()
                    + "$" + (resumeFrom + 1));

            clientConnection.setState(ConnectionState.WAITING);

            clientConnection.getStatistics().setStartTime(
                    System.currentTimeMillis());

        } else {
            logger.debug("Download slot could not be used.");

            logger.debug("Download Slots (" + settings.getUsedDownloadSlots()
                    + "/" + settings.getDownloadSlots() + ")");
     ...
     *            DownloadRequest instance this download is related to
     * @param file
     *            File to download to
     */
    private void download(DownloadRequest downloadRequest, File file,
            boolean normal) {
        ConnectionStatistics stats = clientConnection.getStatistics();

        ConnectionInfo connInfo = clientConnection.getConnectionInfo();

        if (downloadRequest.isResume() && file.exists() && file.isFile()
                && file.canRead()) {

            connInfo.setVerifyResumeSize((int) Math.min(file.length(),
                    ConnectionSettings.VERIFY_RESUME_SIZE));

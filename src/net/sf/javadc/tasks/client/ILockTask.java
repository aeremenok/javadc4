 ...
 * 
 * <p>
 * It is used in the Client-Hub Handshake, the Client-Client Handshake and the
 * Hublist Registration. The other party has to authenticate itself using the
 * LockToKey function. Luckily for us, x has decrypted the algorithm, so now
 * anyone may create a client for the DC network.
 * </p>
 * 
 * @author tw70794
 */
public class ILockTask extends BaseClientTask {
     ...
     */
    public ILockTask(ISettings _settings) {
    ...
    protected final void runTaskTemplate() {
        ...
        if (index > -1) {
            ...
            String debug = "Challenge / Response request received,"
                    + " we're logging in";
            logger.debug(debug);

            try {
                ConnectionInfo connInfo = clientConnection.getConnectionInfo();
                connInfo.setRemoteKey(encryptionHandler
                        .calculateValidationKey(cmdKey));
            ...
            if (clientConnection.getClient() == null) {
                ...
                try {
                    ...
                    clientConnection.requestDirection(
                            ConstantSettings.DOWNLOAD_DIRECTION, false);
            ...
            throw new ClientTaskException("Invalid data " + cmdData
                    + " for $Lock.");

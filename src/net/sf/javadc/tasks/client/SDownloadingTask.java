 ...
 */
public class SDownloadingTask extends BaseClientTask {

    private static final Category logger = Category
            .getInstance(SDownloadingTask.class);
    ...
    protected final void runTaskTemplate() {
        ...
        try {
            ...
            size = (int) Math.min(clientConnection.getReader().available(),
                    stats.getFileLength() - stats.getBytesReceived());
        ...
        if (size > 0) { // something to download

            try {
                continueDownload(size);

            } catch (Exception e) {
                throw new ClientTaskException("Caught "
                        + e.getClass().getName()
        ...
        if (stats.getBytesReceived() == stats.getFileLength()) { // download
            // finished
            ...
            logger.info("Disconnecting after downloading "
                    + (downloadChunkSize / MBYTES) + " mbytes.");
     ...
     */
    private final void continueDownload(int size) {
        ConnectionStatistics stats = clientConnection.getStatistics();
        ...
        try {
            ...
            if (clientConnection.getReader() != null)
                ...
                throw new NullPointerException(
                        "clientConnection.reader was null");

            if (clientConnection.getLocalFile() != null)
                clientConnection.getLocalFile().write(buffer);
            else
                throw new NullPointerException(
                        "clientConnection.localFile was null");

        } catch (IOException io) {
            throw new ClientTaskException("Caught " + io.getClass().getName(),
                    io);
        }

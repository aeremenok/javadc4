...
public class HIGLayout implements LayoutManager2, java.io.Serializable {
    ...
    /**
     * Construct a new layout object. Length of passed arrays define number of
     * columns and number of rows. Each width or height can be less then 0,
     * equal 0 or greater then 0. Passed arrays defines design grid - sizes and
     * dependences between columns and rows. For details see tutorial.
     * 
     * @param widths
    ...
    private Object reallocArray(Object src, int newSize) {
        ...
        System.arraycopy(src, 0, dest, 0, java.lang.reflect.Array
                .getLength(src));
        return dest;
    }
    ...
    public void setColumnWidth(int col, int width) {
        ...
        if (colWidths.length <= col) {
            ...
            colComponents = (ArrayList[]) reallocArray(colComponents,
                    colCount + 3);
            preferredWidths = (int[]) reallocArray(preferredWidths,
                    colCount + 3);
        }
        colWidths[col] = width;
    }
    ...
    public void setRowHeight(int row, int height) {
        ...
        if (rowHeights.length <= row) {
            ...
            heightenWeights = (int[]) reallocArray(heightenWeights,
                    rowCount + 3);
            rowComponents = (ArrayList[]) reallocArray(rowComponents,
                    rowCount + 3);
        }
    ...
    public void setPreferredColumnWidth(int col, int width) {
        if (col > colCount) {
            throw new IllegalArgumentException(
                    "Column index cannot be greater then " + colCount + ".");
        }
    ...
    public void setPreferredRowHeight(int row, int height) {
        if (row > rowCount) {
            throw new IllegalArgumentException(
                    "Column index cannot be greater then " + rowCount + ".");
        }
    ...
    /**
     * Sets weight of specified row. Weight determines distribution of
     * difference when resizing.
     * 
     * @param row
     *            index of row. Index must be > 0.
     ...
     *             <code>addLayoutComponent(Component, Object)</code>. Throws
            ...
            if (colWidths[i] > 0) {
                ...
                if (iComps != null) {
                    for (int j = iComps.size() - 1; j > -1; j--) {
                        ...
                        int width = c.isVisible() ? c.getMinimumSize().width
                                : 0;

                        if (width > 0) {
                            HIGConstraints constr = (HIGConstraints) components
                                    .get(c);
                            if (constr.w < 0)
                                width = -constr.w;
                            else
        ...
        for (int i = 1; i <= rowCount; i++) {
            if (rowHeights[i] > 0) {
                ...
                if (iComps != null) {
                    for (int j = iComps.size() - 1; j > -1; j--) {
                        Component c = (Component) iComps.get(j);
                        int height = c.isVisible() ? c.getMinimumSize().height
                                : invisible;
                        if (height > 0) {
                            HIGConstraints constr = (HIGConstraints) components
                                    .get(c);
        ...
        for (int i = 1; i <= colCount; i++) {
            if (colWidths[i] > 0) {
                ...
                if (iComps != null) {
                    for (int j = iComps.size() - 1; j > -1; j--) {
                        Component c = (Component) iComps.get(j);
                        int width = c.isVisible() ? c.getPreferredSize().width
                                : invisible;
                        if (width > 0) {
                            HIGConstraints constr = (HIGConstraints) components
                                    .get(c);
    ...
    private void distributeSizeDifference(int desiredLength, int[] lengths,
        ...
        double unit = ((double) (desiredLength - preferred))
                / (double) weightSum;

        for (int i = lengths.length - 1; i > 0; i--) {
            newLength = lengths[i] + (int) (unit * (double) weights[i]);
            lengths[i] = (newLength > minLengths[i]) ? newLength
                    : minLengths[i];
        }
    }
     ...
     * x[1] Reference to this array is cached, so data should not be modified.
     */
    ...
    /**
     * returns array of y-coordinates of rows. First coordinate is stored in
     * y[1]. Reference to this array is cached, so data should not be modified.
     */
    ...
    public void layoutContainer(Container target) {
        synchronized (target.getTreeLock()) {
            ...
            for (int i = comps.length - 1; i >= 0; i--) {
                ...
                comp.setBounds((int) compx + c.xCorrection, (int) compy
                        + c.yCorrection, width, height);
            }
        }
    }
     ...
     * constraint object. Constraints object is copied so passed instance can be
     * safely modifed.
     * 
     * @param comp
     *            the component to be added
     * @param HIGConstraints
     *            object determining where/how the component is added to the
     *            layout.
     * @see net.sf.javadc.util.layout.HIGConstraints
     */
        ...
        synchronized (comp.getTreeLock()) {
            ...
            if (constr.x > colCount) {
                throw new RuntimeException(
                        "Column index in constraint object cannot be greater then "
                                + colCount + ".");
            }
            if (constr.x + constr.w - 1 > colCount) {
                throw new RuntimeException(
                        "Width in constraint object cannot be greater then "
                                + (colCount - constr.x + 1) + ".");
            }
            if (constr.y > rowCount) {
                throw new RuntimeException(
                        "Row index in constraint object cannot be greater then "
                                + rowCount + ".");
            }
            if (constr.y + constr.h - 1 > rowCount) {
                throw new RuntimeException(
                        "Height in constraint object cannot be greater then "
                                + (rowCount - constr.y + 1) + ".");
            }

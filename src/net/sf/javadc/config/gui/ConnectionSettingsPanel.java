...
/**
 * <CODE>ConnectionSettingsPanel</CODE> is a panel to edit connection related
 * settings
 * 
 * @author Timo Westk�mper
 */
public class ConnectionSettingsPanel extends AbstractSettingsPanel {
    ...
    private final static Category logger = Category
            .getInstance(ConnectionSettingsPanel.class);
    ...
    /**
     * Create a ConnectionSettingsPanel instance with the given ISettings
     * component
     * 
     * @param _settings
     *            ISettings component to be used
     */
    public ConnectionSettingsPanel(ISettings _settings) {
        if (_settings == null)
        ...
        try {
            ...
            String error = "Caught " + e.getClass().getName()
                    + " when trying to initialize ConnectionSettingsPanel.";
    ...
    /**
     * Enable the force ip and force port text elements, if the Client uses
     * active connection
     * 
     * @param b
     *            true if the Client uses active connections and false, if the
     *            Client uses passive connections
     */
    private final void enableActiveOptions(boolean b) {
    ...
    private final JPanel getActiveSettingsPanel() {
        ...
        p1.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(
                Color.white, new Color(142, 142, 142)), "Active Settings"));
    ...
    private final JPanel getConnectionModePanel() {
        ...
        p1.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(
                Color.white, new Color(142, 142, 142)), "Connection Mode"));
    ...
    private final JPanel getSlotsPanel() {
        ...
        p1.setBorder(new TitledBorder(BorderFactory.createEtchedBorder(
                Color.white, new Color(142, 142, 142)), "Slots"));
    ...
    public final void initPanel() {
        ...
        try {
            txtForcePort.setText(new Integer(settings.getUserInfo().getPort())
                    .toString());
        ...
        try {
            txtUploadSlots.setText(new Integer(settings.getUploadSlots())
                    .toString());
        ...
        try {
            txtDownloadSlots.setText(new Integer(settings.getDownloadSlots())
                    .toString());
    ...
    public final void onOK() {
        if (isChanged()) {
            ...
            settings.getUserInfo().setPort(
                    Integer.parseInt(txtForcePort.getText()));

            settings.setActive(radioActive.isSelected());

            settings.setUploadSlots(new Integer(txtUploadSlots.getText())
                    .intValue());

            settings.setDownloadSlots(new Integer(txtDownloadSlots.getText())
                    .intValue());
...
/*******************************************************************************
 * $Log: ConnectionSettingsPanel.java,v $
 * Revision 1.16  2005/10/02 11:42:28  timowest
 * updated sources and tests
 * Revision 1.15 2005/09/30 15:59:53
 * timowest updated sources and tests
 * 
 * Revision 1.14 2005/09/25 16:40:58 timowest updated sources and tests
 * 

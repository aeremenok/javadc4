...
/**
 * <CODE>DownloadResumePopupMenu</CODE> is a popup menu with download and
 * resume functionality. (Moved out from <CODE>SearchComponent.</CODE>)
 */
public class DownloadResumePopupMenu extends JPopupMenu {
    ...
    private static final Category logger = Category
            .getInstance(DownloadResumePopupMenu.class);
    ...
    /**
     * Create a DownloadResumePopupMenu which uses the given IHub, SearchResult,
     ...
     */
    public DownloadResumePopupMenu(IHub _hub, SearchResult _searchResult,
            List _allSelectedSearchResults, ISettings _settings,
        ...
        if (_hub == null)
            ...
            throw new NullPointerException(
                    "_allSelectedSearchResults was null.");
     ...
     * 
     * @param searchResult
     *            SearchResult for which an available file should be used for
     *            resume
     */
    private final void chooseFileToResume(SearchResult searchResult) {
        final ResumeAsFileChooser fileChooser = new ResumeAsFileChooser(
                settings.getTempDownloadDir());

        if (fileChooser.showResumeDialog(new File(searchResult.getFilename())) == ResumeAsFileChooser.APPROVE_OPTION) {
            logger.debug("Resuming file as: "
                    + fileChooser.getSelectedFile().getAbsolutePath());

            // NOTE : takes only first item into account

            try {
                downloadManager.requestDownload(new DownloadRequest(
                        searchResult, fileChooser.getSelectedFile()
     ...
     * 
     * @param name
     *            filename to be used to resume download
     * @param searchResult
     *            SearchResult to be used for resume
     */
    private final void customResume(String name, SearchResult searchResult) {
        try {
            downloadManager.requestDownload(new DownloadRequest(searchResult,
                    settings.getTempDownloadDir() + File.separator + name,
                    settings));
     ...
     * @param searchResult
     */
    private final void downloadFile(SearchResult searchResult) {
    ...
    /**
     * Search for alternate sources for the selected SearchResults, based on the
     * TTH hash information, if available
     */
    private final void setupSearchForAlternatesMenuItem() {
        ...
        search.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent arg0) {
                ...
                for (Iterator i = allSelectedSearchResults.iterator(); i
                        .hasNext();) {
                    SearchResult searchResult = (SearchResult) i.next();

                    if (searchResult.getTTH() != null) {
                        try {
                            hub.search(new SearchRequest(searchResult.getTTH(),
                                    SearchRequest.TTH, 0, true, false));
                        } catch (IOException e) {
...
/*******************************************************************************
 * $Log: DownloadResumePopupMenu.java,v $
 * Revision 1.22  2005/10/02 11:42:27  timowest
 * updated sources and tests
 * Revision 1.21 2005/09/25 16:40:58
 * timowest updated sources and tests
 * 
 * Revision 1.20 2005/09/14 07:11:49 timowest updated sources
 * 
 * 
 * 
 */

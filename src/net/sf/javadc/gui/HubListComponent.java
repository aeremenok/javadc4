...
/**
 * <CODE>HubListComponent</CODE> provides a searchable table view on the
 * contents of the <CODE>HubList</CODE> and <CODE>HubFavoritesList</CODE>
 * 
...
public class HubListComponent extends JPanel {
    ...
    private static final Category logger = Category
            .getInstance(HubListComponent.class);

    /**
     * 
     */
    private final RowTableModel model = new RowTableModel(new String[] {
            "Name", "Address", "Description", "Country", "Users", "Minshare" });

    /**
     * 
     */
    private final RowTableModel modelFavorites = new RowTableModel(
            new String[] { "Name", "Address", "Description", "Country",
                    "Users", "Minshare" });
     ...
     * 
     */
    private final JPanel hubsTableFavoritesPanel = new JPanel(
            new BorderLayout());
    ...
    /**
     * Create a HubListComponent with the given IHubList, IHubFavoritesList,
     * IHubFactory and ISettings
     * 
     * @param _hubList
     *            IHubList instance to be used
     * @param _hubFavoritesList
     *            IHubFavoritesList instance to be used
     * @param _hubFactory
     *            IHubFactory instance to be used
     */
    public HubListComponent(final IHubList _hubList,
            final IHubFavoritesList _hubFavoritesList,
            final IHubFactory _hubFactory) {
        super(new BorderLayout());

        if (_hubList == null)
            throw new NullPointerException("hubList was null.");
        ...
        hubListTableListener = new HubListTableListener(model,
                hubFavoritesList, hubFactory);

        hubFavoritesTableListener = new HubFavoritesTableListener(
                modelFavorites, hubFavoritesList, hubFactory);

        // hublist

        // uses the filtered table model of the hub list
        hubsTable = new SortableTable(new int[] { 150, 150, -1, 75, 75, 75 },
                hubListTableListener, model, "hubs");

        hubsTable.getTable().setDefaultRenderer(Long.class,
                new ByteCellRenderer());

        // favorite hubs

        hubsTableFavorites = new SortableTable(new int[] { 150, 150, -1, 75,
                75, 75 }, hubFavoritesTableListener, modelFavorites, "hubs");

        hubsTableFavorites.getTable().setDefaultRenderer(Long.class,
                new ByteCellRenderer());

        hostField.addActionListener(new ActionListener() {

            public void actionPerformed(ActionEvent e) {
                hubListTableListener.fireHubSelected(hubFactory
                        .createHub(new HostInfo(hostField.getText())));
        ...
        hubFavoritesList.addListener((HubListListener) Spin
                .over(new HubListListener() {

                    public void hubListChanged() {
                        List h = hubFavoritesList.getHubInfos();

                        IHubInfo[] hubInfos;

                        if (h != null) {
                            hubInfos = (IHubInfo[]) h.toArray(new IHubInfo[h
                                    .size()]);

                        } else {
                            logger
                                    .error("HubInfos from hubFavoritesList was null.");
                        ...
                        for (int i = 0; i < hubInfos.length; i++) {
                            modelFavorites.addRow(hubInfos[i],
                                    getHubColumns(hubInfos[i]));
        ...
        SplitPane splitPane = new SplitPane(SplitPane.VERTICAL_SPLIT,
                hubsTableFavoritesPanel, hubsTablePanel);
    ...
    /**
     * Add the given HubListComponentListener to the HubListTableListener and
     * HubFavoritesTableListener
     * 
     * @param listener
     */
     ...
     * 
     * @param hubInfo
     *            HubInfo instance for which the columns should be retrieved
     * @return
     */
    private final Object[] getHubColumns(IHubInfo hubInfo) {
        if (hubInfo != null) {
            return new Object[] { hubInfo.getName(), hubInfo.getHost(),
                    hubInfo.getDescription(), hubInfo.getCountry(),
...
/*******************************************************************************
 * $Log: HubListComponent.java,v $
 * Revision 1.24  2005/10/02 11:42:28  timowest
 * updated sources and tests
 * Revision 1.23 2005/09/25 16:40:58 timowest
 * updated sources and tests
 * 
 * Revision 1.22 2005/09/14 07:11:49 timowest updated sources
 * 
 * 
 * 
 */

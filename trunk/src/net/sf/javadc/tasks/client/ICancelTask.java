...
/**
 * A downloader can cancel the download gracefully with some clients (NMDC
 * Windows version at least).
 * 
 * <p>
 * It does this by sending $Cancel (without the trailing | character). The
 * uploader then cancels the stream and responds with $Canceled (again, without
 * the trailing |)
 * </p>
 * 
 * @author Timo Westk�mper
 */
public class ICancelTask extends BaseClientTask {
    ...
    protected final void runTaskTemplate() {
        ...
        try {
            if (clientConnection.getState() == ConnectionState.UPLOADING) {
                ...
                throw new ClientTaskException(
                        "$Cancel was send out of context. "
                                + "State should have been UPLOADING, but was "

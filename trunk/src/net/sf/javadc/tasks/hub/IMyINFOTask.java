...
/**
 * <CODE>IMyInfoTask</CODE> contains details about users (share size /
 * connection type / description / email)
 * 
 * <p>
 * This command is part of the Client-Hub Handshake. On login is send after
 * client receive $Hello with own nick. Client resend this on any change. It's
 ...
 * </ul>
 * 
 * <p>// Default NMDC1 connections types 28.8Kbps, 33.6Kbps, 56Kbps, Satellite,
 * ISDN, DSL, Cable, LAN(T1), LAN(T3)
 * </p>
 * <p>// Default NMDC2 connections types Modem, DSL, Cable, Satellite, LAN(T1),
 * LAN(T3)
 * </p>
 * 
 * <p>
 ...
 * </p>
 * 
 * <p>// 1 = normal, 2 and 3 = away, 4 and 5 = server, 6 and 7 = server away, 8
 * and 9 = fireball, 10 and 11 = fireball away
 * </p>
 * <p>// SERVER = client have uptime > 2 hours, > 2 GB shared, upload > 200 MB
 * </p>
 * <p>// fireball = upload > 100 kB/s
 * </p>
 * 
 * <ul>
 ...
 * </ul>
 * 
 * <p>// All characters except $ and | are allowed
 * </p>
 * 
 * <p>// Extended MyINFO, usefull on hubs striping client tags (passive user
 * complains wihtout tags -> without known other users mode).
 * </p>
 * 
 * <p>
 * $MyINFO $ALL &lt;nick&gt;
 ...
 * @author tw70794
 */
public class IMyINFOTask extends BaseHubTask {

    private static final Category logger = Category
            .getInstance(IMyINFOTask.class);
    ...
    protected void runTaskTemplate() {
        ...
        try {
            ...
            logger
                    .error("Caught " + s.getClass().getName() + " for "
                            + cmdData);

            // setHubUserInfo(cmdData);
        } catch (Exception e) {
            throw new HubTaskException("User information " + cmdData
                    + " was invalid.", e);
     ...
     * @param cmdData
     */
    private final void setHubUserInfo(HubUser user, String cmdData) {
        // Strip $ALL_
        ...
        if (index == -1) {
            logger.error("nick description separation could not be found for "
                    + cmdData);
            return;
        }
        ...
        if (elements.length > 2) {
            ...
            logger.error("Not enough arguments  " + elements.length + " in "
                    + cmdData);
     ...
     * @param elements
     */
    private final void setUserInformation(HubUser user, String[] elements) {
        // 3 (raw speed)

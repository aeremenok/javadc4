 ...
 * 
 * <p>
 * XXX dc++ uses the OpList to figure out if it has $Kick and $OpForceMove
 * power, nmdc uses $LogedIn / added &lt;username&gt; the $LogedIn may come
 * before or after the $Hello nmdc doesn't seem to care /sed It really is
 * $LogedIn, not $LoggedIn. If the user is not an Op, $LogedIn is skipped.
 * </p>
 * 
 * @author tw70794
 */
public class ILogedInTask extends BaseHubTask {

    private static final Category logger = Category
            .getInstance(ILogedInTask.class);
     ...
     * @param settings
     */
    public ILogedInTask(ISettings settings) {
    ...
    protected void runTaskTemplate() {
        ...
        if (getCmdData().equals(nick)) {
            logger.info("The local client has been accepted to the Hub "
                    + getHub() + " as an Operator.");

            // getHub().setLoggedIn(true);

        } else {
            throw new HubTaskException("The client nick " + getCmdData()
                    + " supplied in the $LogedIn "

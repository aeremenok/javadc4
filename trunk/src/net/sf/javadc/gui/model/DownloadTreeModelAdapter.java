 ...
 * RequestsModel</CODE> by providing the <CODE>TreeTableModel</CODE>
 * interface on top of it
 * 
 * @author Timo Westk�mper
 */
public class DownloadTreeModelAdapter extends AbstractTreeTableModel implements
        TreeTableModel {

    // private static final String EMPTY_STRING = "";
    private final static Category logger = Category
            .getInstance(DownloadTreeModelAdapter.class);

    private final static String ERROR_MESSAGE = "Not to be used directly from DownloadTreeModelAdapter";

    static final protected String[] cNames = { "Nick", "Name", "Ext", "Path",
            "Address", "Status", "Info" };

    static final protected Class[] cTypes = { TreeTableModel.class,
     ...
     */
    public DownloadTreeModelAdapter(IRequestsModel model) {
        super();
     ...
     */
    public void addRow(IConnection c) {
     ...
     */
    public void deleteRow(IConnection c) {
        throw new RuntimeException(ERROR_MESSAGE);
     ...
     */
    public IConnection getRow(int i) {
    ...
    public Object getValueAt(Object obj, int column) {
        if (obj instanceof IDownloadNode) {
            ...
            String error = "getValue(Object obj, int column) called "
                    + "on object of class " + obj.getClass().getName();
            logger.error(error);
    ...
    public boolean isLeaf(Object node) {
        ...
        if (cl.equals(DownConnectionNode.class)
                || cl.equals(DownRootNode.class)) {
            return false;
    ...
    /** ********************************************************************** */
    private class MyRequestsModelListener implements RequestsModelListener {
        ...
        public void connectionAdded(IConnection connection, int index) {
            logger.debug("Received connectionAdded for connection "
                    + connection);

            Object inserted = downloadNodeFactory
                    .createConnectionNode(connection);

            fireTreeNodesInserted(this, new Object[] { root },
                    new int[] { index }, new Object[] { inserted });
        ...
        public void connectionRemoved(IConnection connection, int index) {
            logger.debug("Received connectionRemoved for connection "
                    + connection);

            Object forRemoval = downloadNodeFactory
                    .createConnectionNode(connection);

            fireTreeNodesRemoved(this, new Object[] { root },
                    new int[] { index }, new Object[] { forRemoval });
        ...
        public void connectionChanged(IConnection connection, int index) {
            logger.debug("Received connectionChanged for connection "
                    + connection);

            Object changed = downloadNodeFactory
                    .createConnectionNode(connection);

            fireTreeNodesChanged(this, new Object[] { root },
                    new int[] { index }, new Object[] { changed });
         ...
         */
        public void requestAdded(IClient client,
                DownloadRequest downloadRequest, int index) {
            logger.debug("Received requestAdded for client " + client
                    + " and DownloadRequest " + downloadRequest);
         ...
         *      net.sf.javadc.net.DownloadRequest, int)
         */
        public void requestRemoved(IClient client,
                DownloadRequest downloadRequest, int index) {
            logger.debug("Received requesRemoved for client " + client
                    + " and DownloadRequest " + downloadRequest);
        ...
        public void requestChanged(IClient client,
                DownloadRequest downloadRequest, int index) {
            logger.debug("Received requestChanged for client " + client
                    + " and DownloadRequest " + downloadRequest);
...
/*******************************************************************************
 * $Log: DownloadTreeModelAdapter.java,v $
 * Revision 1.19  2005/10/02 11:42:28  timowest
 * updated sources and tests
 * Revision 1.18 2005/09/26 17:19:52

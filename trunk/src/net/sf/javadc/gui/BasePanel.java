...
/**
 * <CODE>BasePanel</CODE> is a utility super class which provides application
 * specific icons via a member method.
 * 
...
class BasePanel extends JPanel {
    ...
    private static final String[] imageNames = { "images/32/modem.png",
            "images/32/Satellite.png", "images/32/dsl.png",
            "images/32/Cable.png", "images/32/lan.png", "images/32/server.png",
     ...
     * 
     * @param path
     *            relative file path of the image to be returned
     * @param shaded
     *            whether the image is to be shaded or not
     * @return
     */
    private final ImageIcon getImage(String path, boolean shaded) {
        ImageIcon icon = FileUtils.loadIcon(path);

        if (icon == null)
            throw new NullPointerException("Could not load image for " + path);

        Image image = icon.getImage().getScaledInstance(iconsize, iconsize,
                Image.SCALE_SMOOTH);
...
/*******************************************************************************
 * $Log: BasePanel.java,v $
 * Revision 1.15  2006/06/02 07:46:41  timowest
 * removed obsolete test suites
 *
 * Revision 1.14  2005/10/02 11:42:28  timowest
 * updated sources and tests
 * Revision 1.13 2005/09/30 15:59:53 timowest updated
 * sources and tests
 * 
 * Revision 1.12 2005/09/25 16:40:58 timowest updated sources and tests
 * 
 * Revision 1.11 2005/09/15 17:32:29 timowest added null checks
 * 

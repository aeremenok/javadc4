...
class SearchComponentHubListener extends HubListenerBase {

    private static final Category logger = Category
            .getInstance(SearchComponentHubListener.class);
     ...
     */
    public SearchComponentHubListener(SearchComponent _searchComponent,
            IHub _hub) {
        ...
        new Timer(ConstantSettings.SEARCHRESULTS_UPDATEINTERVAL, taskPerformer)
                .start();
     ...
     */
    public void searchResultAdded(IHub _hub, SearchResult sr,
        ...
        if (searchRequest.matches(sr)) {
            ...
            logger.debug("SearchResult " + sr + " didn't match SearchRequest "
                    + searchRequest);
     ...
     * @param searchRequest
     */
    private void addSearchRequest(IHub _hub, SearchRequest searchRequest) {
        ...
        if (_hub.equals(hub) || isAllHubs) {
            ...
            if (!contained(searchRequest, searchRequests)) {
                ...
                logger
                        .debug("Notified searchComponent that SearchRequest instance "
                                + searchRequest + " was added.");
    ...
    /**
     * Return true if the given SearchRequest is contained in as the first or
     * second item in the given list and return otherwise false
     * 
     * @param sr
     * @param srs
     * @return
     */
    private boolean contained(SearchRequest sr, List srs) {
     ...
     */
    public void __searchRequestAdded(IHub _hub, SearchRequest sr) {
     ...
     */
    public void searchResultsCleared(IHub _hub) {
    ...
    private void update() {
        ...
        synchronized (added) {
            searchResults = (ResultAndRequest[]) added
                    .toArray(new ResultAndRequest[added.size()]);
        ...
        for (int i = 0; i < searchResults.length; i++) {
            ...
            if (j < models.size()) {
                ...
                searchComponent.getTabbedPane().setTitleAt(
                        j,
                        sr.getNamePattern() + " (" + model.getRowCount()
                                + " hits)");
    ...
    private class ResultAndRequest {
        ...
        public ResultAndRequest(SearchResult _searchResult,
...
/*******************************************************************************
 * $Log: SearchComponentHubListener.java,v $
 * Revision 1.17  2005/10/02 11:42:28  timowest
 * updated sources and tests
 * Revision 1.16 2005/09/15 17:32:29

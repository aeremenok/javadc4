...
/**
 * <CODE>MessageComponent</CODE> provides a view on the messages being posted
 * to a <CODE>Hub</CODE>, the local client is connected to. Posting messages
 * is also possible via this component
 * 
 * @author Jesper Nordenberg
 * @version $Revision: 1.20 $ $Date: 2005/10/02 11:42:28 $
 */
public class MessageComponent extends JPanel implements SortableTableListener {
    ...
    private final static Category logger = Category
            .getInstance(MessageComponent.class);
     ...
     * 
     * @param hc
     *            HubComponent to be used
     * @param _showFromColumn
     *            whether the from information is shown
     * @param _from
     * @param settings
     *            ISettings instance to be used
     */
    public MessageComponent(HubComponent hc, boolean _showFromColumn,
            String _from, ISettings settings) {
     ...
     * 
     * @param message
     *            Message instance to be added
     */
    public final void addMessage(Message message) {
        if (!hubComponent.isActive()) {
            logger
                    .debug("HubComponent is not active anymore. Adding of Message aborted.");
            return;
        ...
        try {
            ...
            doc.insertString(doc.getLength(), (firstMessage ? "" : "\n")
                    ...
                    + (showFromColumn ? ("<" + message.getFrom() + "> ") : ""),
                    set);
            ...
            p.setViewPosition(new Point((int) p.getViewPosition().getX(),
                    (int) (p.getView().getHeight() - p.getViewSize()
                            .getHeight())));
    ...
    /**
     * Handler for the input field which sends the typed message to the Hub in
     * general or client specific mode
     */
    private final void onInputField() {
        if (!hubComponent.isActive()) {
            logger
                    .debug("HubComponent is not active anymore. Sending of Message aborted.");
            return;
        ...
        try {
            ...
            if (from == null) {
                ...
                hubComponent.getHub().sendPrivateMessage(inputField.getText(),
                        from);

                String nick = user.getNick();

                Message m = new Message(nick, from, "<" + nick + "> "
                        + inputField.getText());
     ...
     */
    public final void showPopupClicked(int row, int column, MouseEvent e,
            int[] selectedRows) {
...
/*******************************************************************************
 * $Log: MessageComponent.java,v $
 * Revision 1.20  2005/10/02 11:42:28  timowest
 * updated sources and tests
 * Revision 1.19 2005/09/26 17:53:12 timowest
 * added null checks
 * 
 * Revision 1.18 2005/09/25 16:40:58 timowest updated sources and tests
 * 

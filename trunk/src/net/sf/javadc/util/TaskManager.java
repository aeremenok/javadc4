 ...
 */
public class TaskManager implements ITask, Runnable, ITaskManager, Startable {

    private static final Category logger = Category
            .getInstance(TaskManager.class.getName());
            ...
            try {
                if (r[i] != null) {
                    if (logger.isDebugEnabled())
                        logger.debug("About to invoke runTask() on "
                                + r[i].getClass().getName());

                    PerformanceContext cont = new PerformanceContext(
                            "Execution of " + r[i].getClass().getName()
                                    + " instance " + r[i]).start();

                    if (logger.isDebugEnabled())
                        logger.debug("Invocation of "
                                + r[i].getClass().getName() + " finished.");
                    ...
                    logger.error("Instance of class "
                            + r[i].getClass().getName() + " was null.");
 ...
 * Revision 1.17 2005/09/26 17:19:52 timowest updated
 * sources and tests
 * 

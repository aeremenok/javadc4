...
public class ConnectionSettings {
    ...
    /***************************************************************************
     * size in bytes which is reduced from the end position of the file when
     * resuming
     **************************************************************************/
    ...
    static {
        InputStream stream = ConnectionSettings.class.getClassLoader()
        ...
        if (stream != null) {
            ...
            logger.error("InputStream for connection.properties "
    ...
    private static void loadProperties(InputStream stream) throws IOException {
        ...
        UPLOAD_BLOCK_SIZE = Integer.parseInt(props
                .getProperty("UPLOAD_BLOCK_SIZE"));

        SOCKET_INPUT_BUFFER_SIZE = Integer.parseInt(props
                .getProperty("SOCKET_INPUT_BUFFER_SIZE"));

        VERIFY_RESUME_SIZE = Integer.parseInt(props
                .getProperty("SOCKET_INPUT_BUFFER_SIZE"));

        DOWNLOAD_SEGMENT_SIZE = Integer.parseInt(props
                .getProperty("DOWNLOAD_SEGMENT_SIZE"));

        CANCEL_CONNECTION_DELAY = Integer.parseInt(props
                .getProperty("CANCEL_CONNECTION_DELAY"));

        // Client connection related

        CONNECTION_RETRY_INTERVAL = Integer.parseInt(props
                .getProperty("CONNECTION_RETRY_INTERVAL"));

        CONNECTION_WAITING_TIME = Integer.parseInt(props
                .getProperty("CONNECTION_WAITING_TIME"));

        MAX_CONNECTION_COUNT = Integer.parseInt(props
                .getProperty("MAX_CONNECTION_COUNT"));

        // Hub connection related

        HUB_THREAD_INTERRUPTION_TIME = Integer.parseInt(props
                .getProperty("HUB_THREAD_INTERRUPTION_TIME"));

        HUB_THREAD_INTERRUPTION_INTERVAL = Integer.parseInt(props
                .getProperty("HUB_THREAD_INTERRUPTION_INTERVAL"));

        HUB_FLUSHDOWNLOADQUEUE_DELAY = Integer.parseInt(props
                .getProperty("HUB_FLUSHDOWNLOADQUEUE_DELAY"));

        HUB_FLUSHDOWNLOADQUEUE_INTERVAL = Integer.parseInt(props
                .getProperty("HUB_FLUSHDOWNLOADQUEUE_INTERVAL"));
 ...
 * Revision 1.15  2005/10/02 11:42:27  timowest
 * updated sources and tests
 * Revision 1.14 2005/09/30 15:59:52 timowest
 * updated sources and tests
 * 
 * Revision 1.13 2005/09/26 17:19:52 timowest updated sources and tests
 * 

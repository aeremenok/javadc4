...
/**
 * <code>SearchRequestFactory</code> is a pooled factory method implementation
 * to create <code>SearchRequest</code> instances based on query strings
 * 
 * @author Timo Westk�mper
 */
public class SearchRequestFactory extends GenericObjectPool implements
        ISearchRequestFactory {

    private final static Category logger = Category
            .getInstance(SearchRequestFactory.class);

    /**
     * Create a SearchRequestFactory which uses the SearchRequestObjectFactory
     * as the default PoolableObjectFactory
     */
     ...
     */
    public SearchRequest createFromQuery(String searchQuery) {
     ...
     * @return constructed SearchRequest
     */
    public SearchRequest createFromHubRequest(String searchString) {
        SearchRequest sr = createFromQuery(searchString.substring(searchString
                .indexOf(" ") + 1));
        ...
        if (searchString.indexOf("Hub:") > -1) {
            ...
            sr.setRespondAddress(searchString.substring(searchString
                    .indexOf(":") + 1, searchString.indexOf(" ")));

        } else { // it's an active search.
            sr.setActiveMode(true);

            // set the udp-adress as respond adress
            sr.setRespondAddress(searchString.substring(0, searchString
                    .indexOf(" ")));
...
/*******************************************************************************
 * $Log: SearchRequestFactory.java,v $
 * Revision 1.13  2005/10/02 11:42:27  timowest
 * updated sources and tests
 * Revision 1.12 2005/09/15 17:37:14

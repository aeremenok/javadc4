 ...
 */
public class HubManager extends GenericModel implements IHubManager, Startable {

    private final static Category logger = Category
            .getInstance(HubManager.class);
     ...
     */
    public final IHub getHub(int index) {
        if (index < hubs.size())
    ...
    public void addHub(IHub hub) {
        ...
        if (getHub(hub) == null) {
            ...
            logger.debug("Hub " + hub
                    + " has already been added to the HubManager.");
    ...
    public void fireHubAdded(IHub hub) {
        HubManagerListener[] l = (HubManagerListener[]) listenerList
                .getListeners(HubManagerListener.class);
     ...
     */
    public final void removeHub(IHub hub) {
        if (hubs.remove(hub)) {
            fireHubRemoved(hub);
        }
    ...
    public void fireHubRemoved(IHub hub) {
        final HubManagerListener[] l = (HubManagerListener[]) listenerList
                .getListeners(HubManagerListener.class);
     ...
     */
    public final IHub getHubWithIP(String ip) {
        ...
        for (int i = 0; i < h.length; i++) {
            try {
                ...
                String error = "The host for hub " + h[i]
                        + " could not be found.";
     ...
     */
    public final IHub getHub(IHub hub) {
        Hub[] h = (Hub[]) hubs.toArray(new Hub[hubs.size()]);

        for (int i = 0; i < h.length; i++) {
            if (h[i].equals(hub)) {
                logger
                        .debug("Duplicate item " + h[i] + " was found for "
                                + hub);
...
/*******************************************************************************
 * $Log: HubManager.java,v $
 * Revision 1.23  2005/10/02 11:42:28  timowest
 * updated sources and tests
 * Revision 1.22 2005/09/30 15:59:53 timowest updated
 * sources and tests
 * 
 * Revision 1.21 2005/09/25 16:40:58 timowest updated sources and tests
 * 

...
/**
 * <code>HubList</code> represents a list of public hubs, which can be downloaded via HTTP based URL.
 * 
 * @author Jesper Nordenberg
 * @version $Revision: 1.24 $ $Date: 2005/10/02 11:42:28 $
 */
public class HubList
    extends AbstractHubList
    implements
        Runnable
{
    ...
    /**
     * creates a <CODE>HubList</CODE> with the given <CODE>ISettings</CODE> and <CODE>ITaskManager</CODE>
     */
    public HubList(
        ISettings _settings,
        ...
        while ( true )
        {
            ...
            if ( update )
            {
                ...
                catch ( IOException io )
                {
                    ...
                    logger.error( "Error connecting to URL " + settings.getAdvancedSettings().getHublistAddress(), io );
                    // logger.error(io);
    ...
    /**
     * reads the 'BZ' file header from the bzip compressed file and returns true if the correct file header was found
     * and false if not
     */
        ...
        throws IOException
    {
        ...
        try
        {
            hubListAddress = settings.getAdvancedSettings().getHublistAddress().trim();
        ...
        if ( hubListAddress.endsWith( ".bz2" ) )
        {
            ...
            {
                logger.error( "Caught NullPointerException in createHubList()", e );
                // logger.error(e);
                success = false;
            ...
            {
                logger.error( "Caught " + e.getClass().getName() + " in createHubList()", e );
                // logger.error(e);
                success = false;
        ...
        try
        {
            ...
            PerformanceContext cont = new PerformanceContext( "Loading hublist" ).start();
        ...
        throws IOException
    {
        ...
        BufferedReader indata = new BufferedReader( new InputStreamReader( inputStream, defaultCharset ) );
        ...
        ContentHandler contentHandler = new DefaultHandler()
        {
                ...
                Attributes atts )
            {
                ...
                try
                {
                    ...
                    // mandatory elements
                    hubInfo.setAddress( new HostInfo( atts.getValue( "Address" ) + ":" + atts.getValue( "Port" ) ) );
                    ...
                    if ( columns.contains( "Description" ) )
                    {
                        ...
                        if ( atts.getValue( "Description" ).startsWith( "<R" ) )
                        {
                            ...
                            hubInfo.setDescription( atts.getValue( "Description" ).substring( i + 1 ) );
                        }
                        else
                        {
                            hubInfo.setDescription( atts.getValue( "Description" ) );
                    ...
                    if ( columns.contains( "Users" ) && atts.getValue( "Users" ) != null )
                    {
                        hubInfo.setUserCount( Integer.parseInt( atts.getValue( "Users" ) ) );
                    }

                    if ( columns.contains( "Minshare" ) && atts.getValue( "Minshare" ) != null )
                    {
                        hubInfo.setMinshare( Long.parseLong( atts.getValue( "Minshare" ) ) );
                    }

                    if ( columns.contains( "Minslots" ) && atts.getValue( "Minslots" ) != null )
                    {
                        hubInfo.setMinslots( Integer.parseInt( atts.getValue( "Minslots" ) ) );
                    }

                    if ( columns.contains( "Maxhubs" ) && atts.getValue( "Maxhubs" ) != null )
                    {
                        hubInfo.setMaxhubs( Integer.parseInt( atts.getValue( "Maxhubs" ) ) );
                    }

                    if ( columns.contains( "Maxusers" ) && atts.getValue( "Maxusers" ) != null )
                    {
                        hubInfo.setMaxusers( Integer.parseInt( atts.getValue( "Maxusers" ) ) );
                    }
                ...
                catch ( Exception e )
                {
                    ...
                    String[] names = { "Name", "Address", "Port", "Description", "Users" };

                    for ( int i = 0; i < names.length; i++ )
                    {
                        System.out.println( names[i] + " : " + atts.getValue( names[i] ) );
                    }

...
/**
 * <code>AbstractHubList</code> represents an abstract base implementation for
 * the <code>IHubList</code> interface mainly used to separate the listener
 * notification code from the main implementation
 * 
 * @author Timo Westk�mper
 */
public abstract class AbstractHubList extends GenericModel implements IHubList {

    private final static Category logger = Category
            .getInstance(AbstractHubList.class);
    ...
    public void fireHubListChanged() {
        HubListListener[] listeners = (HubListListener[]) listenerList
    ...
    public void setFilter(String _filter) {
        ...
        if (_filter != null) {
            ...
            for (Iterator i = hubInfos.iterator(); i.hasNext();) {
                ...
                if (hubInfo.getDescription().toLowerCase().trim().indexOf(
                        filter) > -1) {

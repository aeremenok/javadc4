...
/**
 * <code>Client</code> represents a remote client with a related queue of
 * downloads (list of <code>DownloadRequests</code>) and a related Client
 * <code>Connection</code>. <code>Client</code> instances are managed via
 * the <code>ClientManager</code>.
 * 
...
public class Client extends AbstractClient implements IClient {
    ...
    /**
     * Create a Client instance with the given HostInfo, ISettings instance and
     * IClientManager
     * 
     * @param _host
     *            HostInfo to be used for the client
     * @param _settings
     *            ISettings instance to be used
     * @param _clientManager
     *            IClientManager instance to be used
     */
    public Client(HostInfo _host, ISettings _settings) {
     ...
     */
    public final void addDownloads(DownloadRequest[] drs) throws IOException {
     ...
     */
    public final void checkDownloads() throws IOException {
        if (!downloads.isEmpty()) { // downloads available
            ...
            DownloadRequest downloadRequest = (DownloadRequest) downloads
                    .get(0);
    ...
    public final void connect() throws IOException {
        if (connection == null) { // connect if not connected
     ...
     */
    public final void serverConnect() throws IOException {
        if (connection == null)
     ...
     */
    public final void clientConnect() throws IOException {
        if (connection == null)
     ...
     */
    public final DownloadRequest getDownload(int index) {

        if (index < downloads.size()) {
            return (DownloadRequest) downloads.get(index);

        } else {
            logger.error("Error in DownloadRequest.getDownload(int index)\n"
    ...
    public final DownloadRequest[] getDownloads() {
        return (DownloadRequest[]) downloads
     ...
     */
    public final void removeDownload(DownloadRequest dr) {
     ...
     */
    public final void removeDownload(int index) {
        ...
        if ((index < 0) || (index >= downloads.size())) {
            ...
            DownloadRequest lastRemoved = (DownloadRequest) downloads
                    .remove(index);
            ...
            if (index == 0) {
                ...
                if (connection != null) {
                    ...
                    logger
                            .error("connection was null in Client.removedDownload(int index)");
                ...
                if (dr != null) {
                    requestChange = lastRemoved.equals(connection
                            .getDownloadRequest());

                } else {
                    logger
                            .info("DownloadRequest was not set for "
                                    + connection);
                ...
                if (requestChange) {
                    if (hasDownloads()) {
                        ...
                        if (connection != null)
                            connection
                                    .setState(ConnectionState.COMMAND_DOWNLOAD);

                    } else {
                        logger.info("Closing connection to " + nick
                                + " because no more downloads are left.");
    ...
    /**
     * Update the states of the DownloadRequests based on the state of the given
     * IConnection instance
     * 
     * @param connection
     *            IConnection whose state is used to determine the states of the
     *            DownloadRequests of this Client
     */
    private final void updateDownloadRequestStates(IConnection connection) {
        ...
        if (connDownloadRequest != null) {
            ...
            for (int i = 0; i < downloads.size(); i++) {
                ...
                DownloadRequestState newState = DownloadRequestState
                        .deriveFromConnectionState(connState,
                                connDownloadRequest.equals(dr));

                // set new state if derived state was defined
                if (newState != null) {
                    dr.setState(DownloadRequestState.deriveFromConnectionState(
                            connState, connDownloadRequest.equals(dr)));
     ...
     */
    public final void setBZSupport(boolean b) {
        bZSupport = b;
    ...
    private class MyConnectionListener implements ConnectionListener {
         ...
         */
        public void downloadComplete(IConnection connection) {
            // removes the finished element from the list of downloads
            DownloadRequest completed = (DownloadRequest) downloads.remove(0);

            logger.debug("DL complete "
                    + completed.getSearchResult().getFilename());

            // finished file is a file list of a user
            if (FileUtils.getExtension(
                    completed.getSearchResult().getFilename()).indexOf("DcLst") > -1) // bad
            {
         ...
         */
        public void downloadFailed(IConnection connection) {
            removeDownload(0);
         ...
         */
        public void uploadComplete(IConnection connection) {
         ...
         */
        public void uploadFailed(IConnection connection) {
...
/*******************************************************************************
 * $Log: Client.java,v $
 * Revision 1.34  2005/10/02 11:42:28  timowest
 * updated sources and tests
 * Revision 1.33 2005/09/30 15:59:53 timowest updated
 * sources and tests
 * 
 * Revision 1.32 2005/09/26 17:19:53 timowest updated sources and tests
 * 
 * Revision 1.31 2005/09/25 16:40:59 timowest updated sources and tests
 * 
 * Revision 1.30 2005/09/14 07:11:50 timowest updated sources
 * 
 * Revision 1.29 2005/09/12 21:12:02 timowest added log block
 * 
 * 
 */

...
/**
 * <CODE>Connection</CODE> is the representation of a connection to a remote
 * client via a TCP Socket
 */
public class Connection extends AbstractConnection implements Runnable, ITask,
        IConnection {
     ...
     * @param _isServer
     *            whether the client acts as a server
     * @param _taskManager
     *            ITaskManager to be used
     * @param _clientConnectionManager
     *            IConnectionManager to be used
     * @param _clientManager
     *            IClientManager to be used
     * @param _clientTaskFactory
     *            IClientTaskFactory to be used
     * 
     * @throws IOException
     */
    public Connection(Client _client, ConnectionListener listener,
            boolean _isServer, ITaskManager _taskManager,
            IConnectionManager _clientConnectionManager,
            IClientManager _clientManager, IClientTaskFactory _clientTaskFactory)
            throws IOException {
    ...
    public final void disconnect() {
        ...
        if (disconnectTask != null) {
            ...
            logger.warn("Already disconnected", new RuntimeException(
                    "Invalid invocation of Connection.disconnect()"));
        }
    ...
    private void establishConnection() {
        ...
        try {
            ...
            logger.error("Caught Exception when running task "
     ...
     * @throws IOException
     */
    private final void processCommand(String cmdString) throws IOException {
        ...
        if ((cmdString.length() > 1) && (sepIndex > 0) // valid command
                && (sepIndex < (cmdString.length() - 1))) {
            ...
            if (cmdStart.startsWith("$")) { // valid command name

                // WARNING : external thread
                IClientTask task = null;

                task = (IClientTask) clientTaskFactory.borrowObject("I"
                        + cmdStart.substring(1));

                if (task != null) { // suitable Task handler was found
                    task.setCmdData(cmdData);
                    task.setClientConnection(this);
                    task.runTask();

                    clientTaskFactory.returnObject("I" + cmdStart.substring(1),
                            task);

                } else { // no Task handler found
                    String warn = "Command unrecognized:" + cmdStart + " "
                            + cmdData + " for state " + getState();
                    logger.warn(warn);
     ...
     *      boolean)
     */
    public final boolean requestDirection(String direction, boolean force)
            throws IOException {
        ...
        if ((currentDirection != null) && currentDirection.equals(direction)
                && !force) { // if we have it, return true.

            return true;

        } else if (((currentDirection == null) || force) && (direction != null)
                && (remoteKey != null)) {
    ...
    public final void run() {
        ...
        if (!additionSuccess) {
            String warn = "ClientConnection " + this + " has already been "
                    + "added to the ConnectionManager.";
            logger.debug(warn);
            ...
            try {
                ...
                if (isServer) {
                    ...
                    socket = new Socket(client.getHost().getHost(), client
                            .getHost().getPort());

                }

                socket
                        .setReceiveBufferSize(ConnectionSettings.SOCKET_INPUT_BUFFER_SIZE);

                // timeout in ms for reading from the InputStream
                // socket.setSoTimeout(2000);

                reader = new TokenInputStream(new BufferedInputStream(socket
                        .getInputStream()), ConstantSettings.COMMAND_END_CHAR);

                writer = new ExtendedBufferedOutputStream(socket
                        .getOutputStream(),
                        ConnectionSettings.UPLOAD_BLOCK_SIZE);
                ...
                if (isServer) {
                    logger.error("Caught " + e.getClass().getName()
                            + " in serverConnect mode", e);

                } else {
                    logger.error("Caught " + e.getClass().getName()
                            + " when trying to obtain socket from "
                            + client.getHost(), e);
                }
    ...
    /**
     * Get the command handler for the given taskString and execute it. If the
     * given taskString is not SDownloading nor Resuming, execute
     * runPassiveTask() in between to catch MaxedOut and Error commands from the
     * external Client which might not follow the choreography
     * 
     * @param taskString
     */
    private void runActiveTask(String taskString) {
        ...
        if (!taskString.equals("SDownloading")
                && !taskString.equals("SResuming")) {
        ...
        if (taskString.equals("SDisconnect")) {
            ...
            IClientTask task = (IClientTask) clientTaskFactory
                    .borrowObject(taskString);
            ...
            logger.warn("Handler for " + taskString
                    + " is not invoked, because state changed from " + oldState
                    + " to " + newState);
    ...
    /**
     * Reads a token from the TokenInputStream, derives a TaskHandler from it
     * and executes the task.
     * 
     * @param useTimeout
     *            switch for timeout based cancellation
     */
    private void runPassiveTask(boolean useTimeout) {
        ...
        try {
            ...
            if (cmd != null) {
                logger.error("Caught " + e.getClass().getName()
                        + " when processing command " + cmd, e);

            } else {
                logger.error("Caught " + e.getClass().getName()
                        + " in runPassiveTask(boolean useTimeout).", e);
     ...
     *      java.lang.String)
     */
    public final void sendCommand(String command, String data) {
        try {
            String towrite = command + ConstantSettings.COMMAND_SEP_CHAR + data
            ...
            String error = "Could not send command " + command + " with data "
                    + data;
            ...
            logger.error("Could not send command " + command + " with data "
                    + data, e);
    ...
    /**
     * Create a new Server Socket and recreate a new Client and add it to the
     * ClientManager
     * 
     * @throws IOException
     */
    private final void serverConnect() throws IOException {
        ...
        HostInfo realHost = new HostInfo(socket.getInetAddress()
                .getHostAddress(), client.getHost().getPort());
     ...
     */
    public final void setDownloadRequest(DownloadRequest request) {
     ...
     */
    public final void setLocalFile(RandomAccessFile file) {
        localFile = file;
     ...
     */
    public final void setReader(TokenInputStream stream) {
        reader = stream;
     ...
     */
    public final void setState(ConnectionState _state) {
     ...
     */
    public final void setWriter(ExtendedBufferedOutputStream stream) {
        writer = stream;
 ...
 * Revision 1.44  2005/10/02 11:42:28  timowest
 * updated sources and tests
 * Revision 1.43 2005/09/30 15:59:53 timowest updated
 * sources and tests
 * 
 * Revision 1.42 2005/09/25 16:40:59 timowest updated sources and tests
 * 
 * Revision 1.41 2005/09/14 07:11:50 timowest updated sources
 * 
 * Revision 1.40 2005/09/12 20:56:35 timowest added log block
 * 
 * 
 */

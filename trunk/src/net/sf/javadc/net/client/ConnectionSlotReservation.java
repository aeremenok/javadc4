...
/**
 * <code>ConnectionSlotReservation</code> is a utility class used by
 * <code>ConnectionManager</code> to manage the download and upload slot
 * reservations of the active Client <code>Connections</code>
 * 
...
public class ConnectionSlotReservation {
    ...
    private static final Category logger = Category
            .getInstance(ConnectionSlotReservation.class);
     ...
     */
    public ConnectionSlotReservation(ISettings _settings) {
     ...
     * 
     * @param conn
     *            Connection for which a download slot was reserved
     * @return true, if freeing was successfull and false, if not
     */
    public boolean freeDownloadSlot(IConnection conn) {
     ...
     * 
     * @param conn
     *            Connection for which a download slot should be reserved
     * @return true, if reservation was successfull and false, if not
     */
    public boolean useDownloadSlot(IConnection conn) {
     ...
     * 
     * @param conn
     *            Connection for which an upload slot has been reserved
     * @return true, if the freeing was successfull and false, if not
     */
    public boolean freeUploadSlot(IConnection conn) {
     ...
     * 
     * @param conn
     *            Connection for which an upload slot should be reserved
     * @return true, if the reservation was successfull and false, if not
     */
    public boolean useUploadSlot(IConnection conn) {
        // if the Connection has not reserved an upload slot
        if (!uploadConnections.contains(conn)) {
    ...
    private void displayInfo() {
        ...
        logger.info("Used slots : " + downloadConnections.size() + " / "
                + uploadConnections.size());
    }
     ...
     */
    public boolean removeConnection(IConnection conn) {
...
/*******************************************************************************
 * $Log: ConnectionSlotReservation.java,v $
 * Revision 1.17  2005/10/02 11:42:28  timowest
 * updated sources and tests
 * Revision 1.16 2005/09/30 15:59:53

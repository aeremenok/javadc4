 ...
 */
public class DownloadSegmentList extends AbstractDownloadSegmentList {

    private static final Category logger = Category
            .getInstance(DownloadSegmentList.class);
    ...
    public DownloadSegmentList(int size, int segmentSize, int fullsize,
        ...
        // are not allowed
        if (size < 2)
            throw new RuntimeException("No segmentation possible for sizes of "
                    + size);
    ...
    /**
     * Create a new segmented DownloadRequest for the given DownloadRequest
     * instance
     * 
     * @param dr
     * @return
    ...
    public DownloadRequest createSegment(DownloadRequest dr) {
        ...
        for (int i = 0; i < requests.length; i++) {
            ...
            if (requests[i] == null) {
                ...
                DownloadRequest newrequest = new DownloadRequest(dr
                        .getSearchResult(), new File(localFilenamePrefix
                        + getSuffix(i + 1, requests.length)), settings);

                Point segment = new Point(i * segmentSize, Math.min((i + 1)
                ...
                if (newrequest.getTempFileSize() < segment.y - segment.x) {
                    ...
                    logger
                            .debug(newrequest.getTempFileSize()
                                    + " is not smaller than "
                                    + (segment.y - segment.x));
                ...
                logger.debug("Slot for segment " + (i + 1)
    ...
    /**
     * Get the filename suffix for the given index and the given full amount of
     * files
     * 
     * @param index
     * @param full
     * @return
     */
    public static String getSuffix(int index, int full) {
     ...
     * @return
     */
    private static final void getSuffix(StringBuffer buffer, int index) {
        if (index < 10)
            buffer.append("00");
        else if (index < 100)
            buffer.append("0");
     ...
     */
    private boolean checkState(DownloadRequest downloadRequest) {

        // if the segment is complete create a new segmented download
        // with the same client information

        int segmentSize = downloadRequest.getSegment().y
                - downloadRequest.getSegment().x;

        logger.debug("Checking file size of "
                + downloadRequest.getLocalFilename());
        ...
        if (downloadRequest.getTempFileSize() == segmentSize) {
            ...
            if (newSegment != null) {
                ...
                logger.debug("No segments left for downloading "
                        + localFilenamePrefix);
            ...
            logger.debug(downloadRequest.getTempFileSize() + " of "
                    + segmentSize + " have been downloaded.");
        }
     ...
     * @param dr
     */
    public void setSlot(int i, DownloadRequest dr) {
        requests[i] = dr;
    }
    ...
    private class DownloadRequestIteration implements ActionListener {
        ...
        public void actionPerformed(ActionEvent arg0) {
            ...
            for (int i = 0; i < requests.length; i++) {
                ...
                File temp = new File(localFilenamePrefix
                        + getSuffix(i + 1, requests.length));
            ...
            if (tempSize == fullsize) {
                logger.info("Merging " + requests.length + " segments of "
                        + localFilenamePrefix);

                // merge the segments and stop the timer

                ITask mergeTask = new SegmentManagerMergeTask(
                        localFilenamePrefix, requests.length);
...
/*******************************************************************************
 * $Log: DownloadSegmentList.java,v $
 * Revision 1.7  2005/10/02 11:42:27  timowest
 * updated sources and tests
 * Revision 1.6 2005/09/30 15:59:53 timowest
 * updated sources and tests
 * 
